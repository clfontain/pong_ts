...
export class Player
{
	...
	constructor (x:number,	y:number, height:number, width:number, color:string, v_y:number, lastKey:string, score:number, id:string, keys: {
		w:boolean,	s:boolean})
	{
		this.x = x;
		this.y = y;
...
export class Ball
{
	...
	speed:number;
	direction:number;
	constructor(x:number, y:number, dx:number, dy:number, rad:number, speed:number, direction:number)
	{
		this.x = x;
		this.y = y;
		this.dx = dx;
...
export class gamestate
{
	...
	circlevsRect(player: Player)
	{
		let px:number = this.ball.x;
		let py:number = this.ball.y;
